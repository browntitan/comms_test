<script lang="ts">
	...
	export let useCaseGuidance = '';    // initial user prompt (if any)
	// ---- ADD THIS export so the parent can pass in 'files' ----
	...
	let prompt = '';
	let chatFiles = [];
	// --------- REMOVED the local "let files = []" here! -----------
	let params = {};

	/**
	 * Keep the parent's systemPrompt in 'params.system', if it's set
	 */
	$: params.system = systemPrompt ? systemPrompt : $settings.system;

	// Use your new "files" prop. Example debug:
	$: console.log('Incoming from parent:', files);

	/**
	 * If we have a chatIdProp, attempt to load the existing chat from the backend
	 */
	$: if (chatIdProp) {
		(async () => {
			loading = true;
			...
			prompt = '';
			// If you want to reset the parent's files array, you can do:
			files = [];
			selectedToolIds = [];
	...
	onMount(async () => {
		...
		if (!chatIdProp && useCaseGuidance.trim().length > 0) {
	...
	const initNewChat = async () => {
		if (Object.keys(history.messages).length === 0 && useCaseGuidance.trim().length > 0) {
			prompt = useCaseGuidance;
		}
		};
