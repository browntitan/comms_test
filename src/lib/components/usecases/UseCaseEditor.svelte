<script lang="ts">
    ...
    onMount(async () => {
        if (use_case) {
            ...
            command = use_case.command.at(0) === '/' ? use_case.command.slice(1) : use_case.command;
            content = use_case.content;

            accessControl = use_case?.access_control ?? null;
        }
    });
</script>
    ...
    >
        <div class="my-2">
            <Tooltip
                content={`${$i18n.t('Only alphanumeric characters and hyphens are allowed')} - ${$i18n.t(
                    'Activate this use case by selecting "/{{COMMAND}}" in the chat input.',
                    {
                        COMMAND: command
                    }
                )}`}
        ...
        <div class="my-2">
            <div class="flex w-full justify-between">
                <div class="self-center text-sm font-semibold">{$i18n.t('Message Template')}</div>
            </div>
            ...
            <div class="mt-2">
                <div>
                    <Textarea
                        ...
                        placeholder={$i18n.t('Write a message template that will be inserted into the chat input.')}
                        bind:value={content}
                        rows={6}
                        required
                    />
