<script lang="ts">
	...
	import SummarizersPanel from '$lib/components/comms6/SummarizersPanel.svelte';
	...
	export let systemPrompt = '';       // Persona's content
	export let useCaseGuidance = '';    // Use Case content
	export let knowledgeBaseIds = [];   // IDs of selected K-Bases
	export let knowledgeBaseNames = []; // Names of selected K-Bases
	
	let useCaseUsed = false; // Flag to track if use case guidance was already used
	...
	$: params.system = systemPrompt ? systemPrompt : $settings.system;
	...
	export async function addSummarizerMessage(summaryText: string, sources?: any[]) {
		// First, check if we're in an existing chat
		if (!$chatId || !history.messages || Object.keys(history.messages).length === 0) {
			// Only initialize if this is truly the first message
			await initChatHandler();
			await tick();
		}

		// Now create and add the summarizer message
		const newId = uuidv4();
		...
		// Update the history
		history.messages[newId] = summarizerMessage;
		
		if (history.currentId && history.messages[history.currentId]) {
			history.messages[history.currentId].childrenIds.push(newId);
		}
		history.currentId = newId;

		// Save only if we have a valid chat ID
	...
	const initNewChat = async () => {
		...
		if ($page.url.pathname.includes('/test/c/')) {
		...
		// If there's no message yet and we haven't used the useCaseGuidance,
		// show it in the input prompt so the user can edit before submitting.
		if (Object.keys(history.messages).length === 0 && !useCaseUsed && useCaseGuidance.trim().length > 0) {
			prompt = useCaseGuidance;
			useCaseUsed = true;
		}
	...
	const initChatHandler = async () => {
		// If we already have a chat ID and it's valid, don't create a new one
		if ($chatId && $chatId !== 'local') {
			return;
		}
		...
		if (!$temporaryChatEnabled) {
			try {
				...
				window.history.replaceState(history.state, '', `/test/c/${chat.id}`);
			} catch (error) {
...
{#if !chatIdProp || (loaded && chatIdProp)}
	<div
	class="h-screen max-h-[100dvh] w-full flex flex-col" id="chat-container"
	>
