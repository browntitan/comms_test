<script lang="ts">
	...
	const initNewChat = async () => {
		...
		if ($page.url.pathname.includes('/comms/c/')) {
	...
	const submitPrompt = async (userPrompt, { _raw = false } = {}) => {
		...
		// // If this is the very first user message, append useCaseGuidance once
		// if (messages.length === 0 && !useCaseUsed && useCaseGuidance.trim().length > 0) {
		//   userPrompt = useCaseGuidance + '\n\n' + userPrompt;
		//   useCaseUsed = true;
		// }
	...
	const sendPromptOllama = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		if (messages.length == 2 && messages.at(-1).content !== '' && selectedModels[0] === model.id) {
			window.history.replaceState(history.state, '', `/comms/c/${_chatId}`);
	...
	const sendPromptOpenAI = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		if (messages.length == 2 && selectedModels[0] === model.id) {
			window.history.replaceState(history.state, '', `/test/c/${_chatId}`);
